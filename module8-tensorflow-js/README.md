...
Similarly to [#training-with-nodejs](training a model from the start with
Node.js), using this approach requires learning Node.js packages to read and
manipulate the dataset used for the transfer learning training. As an
alternative approach, we can start with a [TensorFlow pretained model](https://github.com/tensorflow/models),
